...
- `Alert`: 警告提示组件的包装器。
- `AlertIndicator`: 根据状态属性变化的警告提示的视觉指示器。<sup>(可选)</sup>
- `AlertContent`: 警告的内容主体。<sup>(可选)</sup>
- `AlertTitle`: 警告的标题，可以供屏幕阅读器宣读。<sup>(可选)</sup>
- `AlertDescription`: 警告的描述，可以供屏幕阅读器宣读。<sup>(可选)</sup>
        ...
        <div key={color} className={"flex flex-row items-center justify-between gap-3"}>
          {["solid", "outline", "subtle", "soft"].map((variant) => (
            ...
            >
              ...
              <AlertCloseButton className={"absolute end-2 top-2"} />
            </Alert>
          ))}
        </div>
      ))}
